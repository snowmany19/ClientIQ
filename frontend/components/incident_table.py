...
def render_incident_table(df: pd.DataFrame, user, token):
    ...
    # Filter controls in a card-like container
    with st.container():
        st.markdown("""
        <div style="padding: 15px; border: 1px solid #e0e0e0; border-radius: 8px; background-color: #f8f9fa; margin-bottom: 20px;">
    ...
    if user_role in ["admin", "staff"]:
        ...
        if st.session_state['delete_pending_id'] == selected_incident_id:
            ...
            with st.container():
                ...
                """, unsafe_allow_html=True)
                st.markdown(f"### Delete Incident {selected_incident_id} (Admin/Staff Only)")
