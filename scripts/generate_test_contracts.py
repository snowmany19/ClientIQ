#!/usr/bin/env python3
"""
Generate Test Contracts for ContractGuard.ai
...
CONTRACT_TEMPLATES = {
    "NDA": {
        ...
        "companies": ["TechCorp", "InnovateLabs", "DataSecure", "CloudTech", "FutureSystems"],
    ...
    "MSA": {
        ...
        "companies": ["ServicePro", "ConsultingCorp", "ExpertSolutions", "ProfessionalPartners", "ServiceHub"],
    ...
    "SOW": {
        ...
        "companies": ["ProjectCorp", "BuildTech", "DesignSolutions", "ImplementationPro", "ProjectHub"],
    ...
    "Employment": {
        ...
        "companies": ["EmployCorp", "WorkTech", "CareerSolutions", "JobHub", "EmploymentPro"],
    ...
    "Vendor": {
        ...
        "companies": ["VendorCorp", "SupplyTech", "ProviderSolutions", "VendorHub", "SupplyPro"],
    ...
    "Lease": {
        ...
        "companies": ["LeaseCorp", "RentTech", "PropertySolutions", "LeaseHub", "RentPro"],
    ...
    "Other": {
        ...
        "companies": ["OtherCorp", "BusinessTech", "PartnershipSolutions", "OtherHub", "BusinessPro"],
...
def generate_contract_content(category: str) -> Dict[str, Any]:
    ...
    company2 = random.choice(template["companies"])
...
1. PREMISES
Landlord hereby leases to Tenant the {property_type} located at {fake.address()}.
...
def create_contract(token: str, contract_data: Dict[str, Any]) -> int:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Created contract: {contract_data['title']} (ID: {contract_id})")
            return contract_id
        else:
...
def upload_contract_file(token: str, contract_id: int, contract_data: Dict[str, Any]) -> bool:
    """Upload contract file content."""
    try:
        headers = {"Authorization": f"Bearer {token}"}
        
        # Create a temporary file with contract content
        filename = f"contract_{contract_id}_{contract_data['category']}_{fake.file_name(extension='txt')}"
        ...
        if response.status_code == 200:
            print(f"✅ Uploaded file for contract {contract_id}")
            return True
        else:
...
def analyze_contract(token: str, contract_id: int) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"✅ Analysis triggered for contract {contract_id}")
            return True
        else:
...
def main():
    """Main function to generate and process test contracts."""
    print("🚀 ContractGuard.ai - Test Contract Generator")
    print("=" * 50)
    ...
    for category in categories:
        ...
        for i in range(num_contracts):
            ...
            if upload_contract_file(token, contract_id, contract_data):
                # Trigger analysis
                analyze_contract(token, contract_id)
            
            # Small delay between contracts
            time.sleep(1)
    
    print(f"\n🎉 Test contract generation complete!")
    print(f"📊 Generated contracts across all {len(categories)} categories")
    print(f"🔍 Check the dashboard to see the analysis results")
